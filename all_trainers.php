            ...
            <div class="col-md-9 main-text">
                ...
                <div class="admin-right">
                    ...
                    if (isset($_GET['del_id'])) { //проверяем, есть ли переменная
                        ...
                        $rows = mysqli_query($mysqli, "DELETE FROM `Trainers` WHERE `id` = {$_GET['del_id']}");
                        if ($rows) {
                            echo "<p style='background: rgb(248, 191, 152)' >Тренер был удален!</p>";
                        } else {
                    ...
                    if (isset($_POST["name"])) {
                        ...
                        if (isset($_GET['red_id'])) {
                            $rows = mysqli_query($mysqli, "UPDATE `Trainers` SET `surname` = '{$_POST['surname']}',`name` = '{$_POST['name']}',`patronymic` = '{$_POST['patronymic']}',`sport` = '{$_POST['sport']}',`tel` = '{$_POST['tel']}' WHERE `id`={$_GET['red_id']}");
                        } else {
                            //Иначе вставляем данные, подставляя их в запрос
                            $rows = mysqli_query($mysqli, "INSERT INTO `Trainers` (`surname`, `name`, `patronymic`, `sport`, `tel`) VALUES ('{$_POST['surname']}', '{$_POST['name']}', '{$_POST['patronymic']}', '{$_POST['sport']}', '{$_POST['tel']}')");
                        }
                    ...
                    if (isset($_GET['red_id'])) {
                        $rows = mysqli_query($mysqli, "SELECT `id`, `surname`, `name`, `patronymic`, `sport`, `tel` FROM `Trainers` WHERE `id`={$_GET['red_id']}");
                        $product = mysqli_fetch_array($rows);
                    }
                    //////////////// end ВОТ ТУТ РЕДАКТИРУЕМ СТРОКУ
                    ?>
                    <h4 style="margin-bottom: 0 ; ">Редактировать данные тренера:</h4>
                    ...
                    <table border="2" style="border:2 #8a8a8a;">
                        <tr>
                            ...
                            <td style="width: 170px; background: rgb(215, 248, 152); text-align: center">Фамилия</td>
                            <td style="width: 170px; background: rgb(215, 248, 152); text-align: center">Имя</td>
                            <td style="width: 170px; background: rgb(215, 248, 152); text-align: center">Отчество</td>
                            <td style="width: 250px; background: rgb(215, 248, 152); text-align: center">Вид спорта</td>
                        ...
                        <?php
                        $rows = mysqli_query($mysqli, "SELECT id, surname, name, patronymic, sport, tel FROM `Trainers`");
